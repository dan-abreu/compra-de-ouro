...
export function SalesPage() {
  ...
  const [pendingPayload, setPendingPayload] = useState<SalesPayload | null>(null);
  const [pendingAuditPayload, setPendingAuditPayload] = useState<SalesAuditPayload | null>(null);
  ...
  const validateOrder = () => {
    ...
    if (!canFinalize) {
      setGlobalError("A venda so pode ser finalizada quando a soma exata dos splits em USD fechar o total da ordem e todos os campos obrigatorios estiverem preenchidos.");
      return false;
    }
  ...
  const buildPayloads = (): { normalizedPayload: SalesPayload; treasuryAuditPayload: SalesAuditPayload } => {
    const normalizedPayload: SalesPayload = {
    ...
    const treasuryAuditPayload: SalesAuditPayload = {
      ...normalizedPayload,
      totalOrderValueUsd: decimalText(totalOrderValueUsd),
      paymentSplits: paymentLines.map((line) => {
        const preview = paymentPreview.find((item) => item.id === line.id);
  ...
  const confirmAndSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await apiRequest("/orders/sale", "POST", pendingPayload);
          ...
          <form onSubmit={submit} className="space-y-5">
                ...
                <div className="xl:col-span-2">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <HelpHint
                        title="Contraparte da Venda"
                        content="Selecione quem esta comprando ouro da loja. Se nao for cadastrado, ative Fornecedor Avulso."
                      />
                    </div>
                    <label className="flex items-center gap-2 rounded-xl border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-800">
                      ...
                      Fornecedor Avulso
                      <HelpHint
                        title="Fornecedor Avulso"
                        content="Use quando nao houver cadastro. Em valores altos, a politica de compliance pode exigir bloqueio."
                      />
                    </label>
                  </div>
                  <TradePartySelector
                    ...
                    disabled={isWalkIn}
                    placeholder={isWalkIn ? "Fornecedor Avulso" : "Buscar fornecedor"}
            ...
            <div className="rounded-2xl border border-stone-300/70 bg-stone-50 p-3">
              <div className="flex items-center gap-2">
                ...
                <HelpHint
                  ...
                  content="Classifica o tipo de ouro vendido para manter o controle de estoque e a rastreabilidade da operacao."
                />
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <label className="text-xs font-medium text-stone-700">
                <span className="mb-1 flex items-center gap-2">
                  ...
                  <HelpHint
                    title="Peso do Ouro"
                    content="Peso fisico da venda. Multiplicado pelo preco por grama para formar o total da ordem."
                  />
                </span>
              ...
              <label className="text-xs font-medium text-stone-700">
                <span className="mb-1 flex items-center gap-2">
                  ...
                  <HelpHint
                    ...
                    content="Percentual informado na venda para documentacao tecnica e analise do negocio."
                  />
                </span>
              ...
              <label className="text-xs font-medium text-stone-700">
                <span className="mb-1 flex items-center gap-2">
                  ...
                  <HelpHint
                    ...
                    content="Preco final negociado na venda. Pode usar o spot como referencia e ajustar conforme a negociacao."
                  />
                </span>
                <input
            ...
            {needsSoftComplianceWarning ? (
              <div className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                <p className="font-semibold">Compliance: fornecedor avulso acima de USD 10.000,00.</p>
            ...
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-stone-600">
              ...
              <HelpHint
                ...
                content="Distribua o recebimento por moeda. A venda so finaliza quando o total convertido em USD fecha exatamente."
              />
            </div>
          ...
          <Card title="Snapshot de Mercado">
            <div className="mb-3 flex items-center gap-2 text-xs text-stone-600">
              ...
              <HelpHint
                ...
                content="Mostra cotacoes usadas como referencia operacional para precificar a venda."
              />
            </div>
          ...
          <Card title="Resumo Operacional">
            <div className="mb-3 flex items-center gap-2 text-xs text-stone-600">
              ...
              <HelpHint
                ...
                content="Checklist da venda com contraparte, modo da ordem e status do rateio antes da confirmacao."
              />
            </div>
            <div className="space-y-3 text-sm text-stone-700">
              <p><strong>Fornecedores carregados:</strong> {loading ? "carregando" : supplierCount}</p>
              <p><strong>Modo da ordem:</strong> {isWalkIn ? "Fornecedor Avulso" : "Fornecedor cadastrado"}</p>
              <p><strong>Fornecedor selecionado:</strong> {selectedSupplier?.displayName ?? "-"}</p>
              <p><strong>Contato principal:</strong> {selectedSupplier?.contactName ?? "-"}</p>
            ...
            {activeModal === "compliance" ? (
              <>
                ...
                <p className="mt-2 text-sm text-stone-700">
                  Esta venda está acima de USD 10.000,00 em modo avulso. Deseja continuar para conferência final ou cancelar agora?
                </p>
              ...
              <>
                <h3 className="font-heading text-lg font-semibold text-stone-900">Recibo de Conferência da Venda</h3>
                <div className="mt-3 max-h-[60vh] space-y-4 overflow-y-auto rounded-xl border border-stone-200 bg-stone-50 p-3 text-sm">
                  ...
                  <p><strong>Contraparte:</strong> {isWalkIn ? "Fornecedor Avulso" : selectedSupplier?.displayName ?? "-"}</p>
                  ...
                  <div>
                    <p className="mb-2 font-semibold text-stone-800">Rateio de recebimento</p>
